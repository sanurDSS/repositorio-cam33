...
/**
 * Esta clase divide un conjunto de puntos en dos subconjuntos. Para la división utiliza un plano vertical
 * paralelo al plano XY. Esta es otra de esas clases que permite una ejecucion interactiva en funcion de
 * un parametro Manual. En ese caso, se utiliza un cuadro de dialogo para seleccionar manualmente la
 * posicion del plano de división en el eje Z. Para que funcione automáticamente, habrá que especificar
 * mediante el parámetro Width la posición del plano de división en el eje Z.
 */
public class FDivideDepth extends Function3D {
	...
	public FDivideDepth() {
		...
		p1.setValue(true);
		FunctionParam p2 = new FunctionParam("Depth", ParamType.FLOAT);
		p2.setValue(0.0);
	...
	public void proccessData(ScreenData scr_data) throws JIPException {
		...
		depth = paramValueReal("Depth");

		if(manual)
		{
			depth = scr_data.getMinRango()[2];
		...
		if(!manual || div_diag.isConfirmed())
		{
			...
			scr_new.name = "closer_" + scr_data.name;
			elements = sets.get(1);
			...
			scr_new = new PointSet3D(new ScreenOptions());
			scr_new.name = "further_" + scr_data.name;
			elements = sets.get(0);
			for(cont=0;cont<elements.length;cont++)
			{
				point = (Point3D)elements[cont];
	...
	{
		private static final long serialVersionUID = -4176421774950236914L;
		...
		public DivideDialog(ScreenData scr_data)
		{
			...
			paux = new JPanel();
			thres = new JLabel(" "+depth);
			paux.add("West", new JLabel("Threshold: "));
			paux.add("East", thres);
			pmain.add(paux);

			slider = new JSlider(JSlider.HORIZONTAL, (int)(scr_data.getMinRango()[2]*100), (int)(scr_data.getMaxRango()[2]*100), (int)(depth*100));
			slider.addChangeListener(this);
			pmain.add(slider);

			paux = new JPanel();
			utag = new JLabel(s2 + " points  ");
			paux.add("West", new JLabel("  Closer set contains: "));
			paux.add("East", utag);
			pmain.add(paux);

			paux = new JPanel();
			btag = new JLabel(s1 + " points");
			paux.add("West", new JLabel("Further set contains: "));
			paux.add("East", btag);
			pmain.add(paux);
