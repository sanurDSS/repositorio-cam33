 ...
 * paralelo al plano YZ. Esta es otra de esas clases que permite una ejecucion interactiva en funcion de
 * un parametro Manual. En ese caso, se utiliza un cuadro de dialogo para seleccionar manualmente la
 * posicion del plano de división en el eje X. Para que funcione automáticamente, habrá que especificar
 * mediante el parámetro Width la posición del plano de división en el eje X.
 */
public class FDivideVert extends Function3D {
	...
	public FDivideVert()
	{
		...
		p1.setValue(true);
		FunctionParam p2 = new FunctionParam("Width", ParamType.FLOAT);
		p2.setValue(0.0);
	...
	public void proccessData(ScreenData scr_data) throws JIPException {
		...
		width = paramValueReal("Width");

		if(manual)
		{
			width = scr_data.getMinRango()[0];
	...
	class DivideDialog extends JDialog implements ActionListener, ChangeListener
	{
		...
		public DivideDialog(ScreenData scr_data)
		{
			...
			slider = new JSlider(JSlider.HORIZONTAL, (int)(scr_data.getMinRango()[0]*100), (int)(scr_data.getMaxRango()[0]*100), (int)(width*100));
