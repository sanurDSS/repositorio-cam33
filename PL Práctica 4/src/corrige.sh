...
# Sin par�metros, eval�a la pr�ctica con todos los ficheros de prueba (*.fnt) 
# del directorio actual.
#
# Si se indica un par�metro, como "./corrige.sh 050", eval�a la pr�ctica con
# el fichero 050.fnt.

# Ubicaci�n de ANTLR:
ANTLR_CLASSPATH_AUTOCORRECTOR=autocorrector/antlrworks-1.3.1.jar
...
# Nombre del fichero con la especificaci�n de ANTLR:
nom=plp4
...
if test ! -z $1
then  
  archivosentrada=autocorrector/$1.fnt
  mostrardiff="si"
else
  archivosentrada=$(ls autocorrector/*.fnt)
fi
...
rm -f autocorrector/*.tmp
rm -f autocorrector/*.tmp.exe

exit 0


#cambios: meter la palabra "autocorrector" en algunos sitios para que encuentre los ficheros que ahora est�n en un directorio
