...
do
  n=$(basename $i .fnt)
  n=autocorrector/$n  
  rm -f $n.s.tmp $n.err.tmp $n.tmp.exe	
  if test $TARGET = "java"
  then
    $java  -classpath $ANTLR_CLASSPATH_AUTOCORRECTOR:. Main $n.fnt >$n.tmp.s 2> $n.err.tmp
  fi
      
  if [ -s $n.tmp.s ]
  then
   gcc -o $n $n.tmp.s >/dev/null 2<&1
   $(./$n <$n.ent  > $n.sal.tmp)
   rm -f $n
  else
   rm -f $n.sal.tmp
...
rm -f autocorrector/*.tmp.s
rm -f autocorrector/*.err.tmp
rm -f autocorrector/*.sal.tmp
